...
export default function BusinessActions({ businessId, businessName }: BusinessActionsProps) {
  const [copied, setCopied] = useState(false);

  const handleShare = async () => {
    const shareUrl = typeof window !== 'undefined' ? window.location.href : `https://menuland.net/business/${businessId}`;
    
    if (navigator.share) {
      try {
        await navigator.share({
          title: `${businessName} - Menuland`,
          text: `${businessName} dijital menüsünü Menuland'de inceleyin!`,
          url: shareUrl,
        });
      } catch (err) {
